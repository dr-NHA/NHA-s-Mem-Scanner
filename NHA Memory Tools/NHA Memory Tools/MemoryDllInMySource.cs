...
namespace Memory
{
    ...
    public struct AOB_AddressResult
    {
    ...
    public class Mem
    {
        ...
        public void FreezeValue(string address, string type, string value, string file = "")
        {
            ...
            lock (FreezeTokenSrcs)
            {
                ...
                {
                    MessageBox.Show("Changing Freezing Address " + address + " Value " + value);
                    try
                    {
                    ...
                    {
                        MessageBox.Show("ERROR: Avoided a crash. Address " + address + " was not frozen.");
                    }
                }
                else
                    MessageBox.Show("Adding Freezing Address " + address + " Value " + value);
        ...
        {
            MessageBox.Show("Un-Freezing Address " + address);
            try
            {
                lock (FreezeTokenSrcs)
                {
            ...
            {
                MessageBox.Show("ERROR: Address " + address + " was not frozen.");
            }
        }
...
//if (!IsAdmin()){DebugBox("WARNING: You are NOT running this program as admin! Visit https://github.com/erfg12/memory.dll/wiki/Administrative-Privileges");}
if (pid <= 0){ //MessageBox.Show("ERROR: OpenProcess given proc ID 0.");
return false;}
if (theProc != null && theProc.Id == pid) { return true;}
try{
theProc = Process.GetProcessById(pid);
if (theProc != null && !theProc.Responding){
//MessageBox.Show("ERROR: OpenProcess: Process is not responding or null.");
return false;
}
pHandle = OpenProcess(0x1F0FFF, true, pid);
//Process.EnterDebugMode();
//MessageBox.Show(pHandle.ToString());
if (pHandle == IntPtr.Zero){
var eCode = Marshal.GetLastWin32Error();
//MessageBox.Show("ERROR: OpenProcess has failed opening a handle to the target process (GetLastWin32ErrorCode: " + eCode + ")");
...
//MessageBox.Show("Program is operating at Administrative level. Process #" + theProc + " is open and modules are stored.");
return true;
}catch (Exception e) {
...
return false;
        ...
        public bool IsAdmin()
        {
            ...
            {
                MessageBox.Show("ERROR: Could not determin if program is running as admin. Is the NuGet package \"System.Security.Principal.Windows\" missing?");
                return false;
            }
        }
...
//  MessageBox.Show("WARNING: Game is x64, but your Trainer is x86! You will be missing some modules, change your Trainer's Solution Platform.");
 }else if (!_is64Bit && IntPtr.Size == 8){
//     MessageBox.Show("WARNING: Game is x86, but your Trainer is x64! You will be missing some modules, change your Trainer's Solution Platform.");
}
...
//MessageBox.Show("Found " + modules.Count() + " process modules.");
}

public void SetFocus(){
int style = GetWindowLong(theProc.MainWindowHandle, -16);
if ((style & 0x20000000) == 0x20000000) //minimized
 SendMessage(theProc.Handle, 0x0112, (IntPtr)0xF120, IntPtr.Zero);
SetForegroundWindow(theProc.MainWindowHandle);
        ...
        public string LoadCode(string name, string iniFile)
        {
            ...
            if (iniFile != "")
            {
                if (File.Exists(iniFile))
                    ...
                    MessageBox.Show("ERROR: ini file \"" + iniFile + "\" not found!");
            }
            else
        ...
        private int LoadIntCode(string name, string path)
        {
            ...
            {
                MessageBox.Show("ERROR: LoadIntCode function crashed!");
                return 0;
            }
        }
        ...
        public bool WriteMemory(string code, string type, string write, string file = "", System.Text.Encoding stringEncoding = null, bool RemoveWriteProtection = true)
        {
            ...
            {
                float Parsed;
                if (!float.TryParse(write,out Parsed)) { return false; }
                write = Convert.ToString(Parsed);
            ...
            {
                int Parsed;
                if (!int.TryParse(write, out Parsed)) { return false; }
                write = Convert.ToString(Parsed);
            ...
            else if (type.ToLower() == "bytes")
            {
                if (write.Contains(",") || write.Contains(" ")) //check if it's a proper array
                {
                    ...
                    if (write.Contains(","))
                        ...
                        stringBytes = write.Split(' ');
                    //MessageBox.Show("write:" + write + " stringBytes:" + stringBytes);
        ...
        public UIntPtr GetCode(string name, string path = "", int size = 8)
        {
            ...
            if (Is64Bit)
            {
                //MessageBox.Show("Changing to 64bit code...");
            ...
            if (theCode == "")
            {
                //MessageBox.Show("ERROR: LoadCode returned blank. NAME:" + name + " PATH:" + path);
                return UIntPtr.Zero;
            }
            else
            {
                //MessageBox.Show("Found code=" + theCode + " NAME:" + name + " PATH:" + path);
            }
            ...
            if (newOffsets.Contains(','))
            {
                ...
                else if (!theCode.Contains("base") && !theCode.Contains("main") && theCode.Contains("+"))
                {
                    ...
                    else
                    {
                        ...
                        {
                            MessageBox.Show("Module " + moduleName[0] + " was not found in module list!");
                            MessageBox.Show("Modules: " + string.Join(",", modules));
                        }
                    }
            ...
            else // no offsets
            {
                ...
                IntPtr altModule = IntPtr.Zero;
                //MessageBox.Show("newOffsets=" + newOffsets);
        ...
        {
            AoB_OutInfo = "From Root Aob: " + search;
        ...
        /// <param name="executable">Include executable addresses in scan</param>
        /// <returns>IEnumerable of all addresses found.</returns>
        public Task AoBScan(ulong start, ulong end, string search, bool writable = false, bool executable = true, string file = "")
        {
...
public  List<MemoryRegionResult> BetterViewByNHA(ulong start=(ulong)0, ulong end=(ulong)99999999999, bool readable=true, bool writable=true, bool executable=true){
            ...
            ulong proc_min_address = sys_info.minimumApplicationAddress.ToUInt64();
            ulong proc_max_address = sys_info.maximumApplicationAddress.ToUInt64();
            ...
            UIntPtr currentBaseAddress = new UIntPtr(start);

            MEMORY_BASIC_INFORMATION memInfo = new MEMORY_BASIC_INFORMATION();

            //MessageBox.Show("[DEBUG] start:0x" + start.ToString("X8") + " curBase:0x" + currentBaseAddress.ToUInt64().ToString("X8") + " end:0x" + end.ToString("X8") + " size:0x" + memInfo.RegionSize.ToString("X8") + " vAloc:" + VirtualQueryEx(pHandle, currentBaseAddress, out memInfo).ToUInt64().ToString());
            ...
            while (VirtualQueryEx(pHandle, currentBaseAddress, out memInfo).ToUInt64() != 0 &&
                ...
                isValid &= memInfo.BaseAddress.ToUInt64() < proc_max_address;
        ...
        /// <param name="executable">Include executable addresses in scan</param>
        /// <returns>IEnumerable of all addresses found.</returns>
        public string AoB_OutInfo = "";
        public List<AOB_AddressResult> AoB_OutAddys = new List<AOB_AddressResult>();
public Task AoBScan(ulong start, ulong end, string search, bool readable, bool writable, bool executable, string file = ""){
return Task.Run(() => {
    AoB_OutInfo = "AoB Started!";
var memRegionList = new List<MemoryRegionResult>();
...
AoB_OutInfo = "Scanning Memory Regions!";
...
if (start > proc_max_address){ start = proc_min_address; }

if (end > proc_max_address){ end =proc_max_address; }
UIntPtr currentBaseAddress = new UIntPtr((ulong)start);
MEMORY_BASIC_INFORMATION memInfo = new MEMORY_BASIC_INFORMATION();
//MessageBox.Show("[DEBUG] start:0x" + start.ToString("X8") + " curBase:0x" + currentBaseAddress.ToUInt64().ToString("X8") + " end:0x" + end.ToString("X8") + " size:0x" + memInfo.RegionSize.ToString("X8") + " vAloc:" + VirtualQueryEx(pHandle, currentBaseAddress, out memInfo).ToUInt64().ToString());
while (VirtualQueryEx(pHandle, currentBaseAddress, out memInfo).ToUInt64() != 0 && currentBaseAddress.ToUInt64() < (ulong)end && currentBaseAddress.ToUInt64() + (ulong)memInfo.RegionSize > currentBaseAddress.ToUInt64()){
bool isValid = memInfo.State == MEM_COMMIT;
isValid &= memInfo.BaseAddress.ToUInt64() < proc_max_address;
isValid &= ((memInfo.Protect & PAGE_GUARD) == 0);
isValid &= ((memInfo.Protect & PAGE_NOACCESS) == 0);
...
if (!isValid) { currentBaseAddress = new UIntPtr(memInfo.BaseAddress.ToUInt64() + (ulong)memInfo.RegionSize); continue; }

MemoryRegionResult memRegion = new MemoryRegionResult{CurrentBaseAddress = currentBaseAddress,RegionSize = memInfo.RegionSize,RegionBase = memInfo.BaseAddress};
...
if ((long)previousRegion.RegionBase + previousRegion.RegionSize == (long)memInfo.BaseAddress){
memRegionList[memRegionList.Count - 1] = new MemoryRegionResult{CurrentBaseAddress = previousRegion.CurrentBaseAddress,RegionBase = previousRegion.RegionBase,RegionSize = previousRegion.RegionSize + memInfo.RegionSize};
continue;
}
}
memRegionList.Add(memRegion);
...
string Cap= "/" + memRegionList.Count.ToString();
string MEX= "Scanning Memory Through Regions! ";
    List<MemoryRegionResult> REG = new List<MemoryRegionResult>();
    int Current = 0;
void SD(MemoryRegionResult item){
if(!REG.Contains(item)){
REG.Add(item);
AoB_OutInfo = MEX+Current+Cap; Current++;
    }}
AoB_OutInfo = MEX+Current+Cap; 

var pb = false;
Parallel.ForEach(memRegionList, (item, parallelLoopState, index) => {
if (pb == false&&item.RegionSize>(long)MemoryRegionSizes[0]&& item.RegionSize<(long)MemoryRegionSizes[1]){
long[] compareResults = CompareScan(item, aobPattern, mask);
foreach (long result in compareResults){try{
AOB_AddressResult RES=new AOB_AddressResult();
RES.Address= result.ToString("X");
RES.RegionSize = item.RegionSize;
RES.BaseAddress= item.RegionBase.ToUInt64().ToString("X2");
    SD(item);
AoB_OutAddys.Add(RES);
}catch{}
if(OnlyFindOneAddress==true){
pb=true;
break;}
}
}
...
AoB_OutInfo = "Scanning Memory Complete!";


});
}

public bool OnlyFindOneAddress=false;
public long[] MemoryRegionSizes = {0,111111111110 };


public string OutKnownMemRegionInf="";
